  ...
  function processCommits(data) {
    return d3
      ...
      .map(([commit, lines]) => {
        ...
        let ret = {
          id: commit,
          url: 'https://github.com/YadushanUCSD/portfolio/commit/' + commit,
          author,
          date,
          time,
          timezone,
...
let files = d3
  ...
  filesContainer.select('dt').html((d) => `
    <code>${d.name}</code>
    <small style="display: block; opacity: 0.6; font-size: 0.8em; margin-top: 4px;">
...
d3.select('#scatter-story')
  ...
  function onStepEnter(response) {
    ...
    commitProgress = Math.min(100, Math.max(0, commitProgress));
    const slider = document.getElementById('commit-progress');
